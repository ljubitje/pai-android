...
export async function runCLI(): Promise<void> {
  ...
  try {
    ...
    const rcFile = ((s: string) => s.includes("bash") ? ".bashrc" : s.includes("fish") ? ".config/fish/config.fish" : ".zshrc")(process.env.SHELL || "/bin/zsh");
    print(`  ${c.gray}Run ${c.bold}source ~/${rcFile} && pai${c.reset}${c.gray} to launch PAI.${c.reset}`);
    print("");
