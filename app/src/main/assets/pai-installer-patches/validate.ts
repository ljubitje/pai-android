...
export async function runValidation(state: InstallState): Promise<ValidationCheck[]> {
  ...
  // 4. PAI skill present (moved from skills/PAI/SKILL.md to PAI/SKILL.md in v4.0.3)
  const skillPath = join(paiDir, "PAI", "SKILL.md");
  checks.push({
    name: "PAI core skill",
    passed: existsSync(skillPath),
    detail: existsSync(skillPath) ? "Present" : "Not found — missing PAI/SKILL.md",
    critical: false,
  });
  ...
  if (existsSync(rcPath)) {
    try {
      ...
      aliasConfigured = rcContent.includes("# PAI alias") && rcContent.includes("alias pai=");
    } catch {}
  }
  ...
  checks.push({
    ...
    detail: aliasConfigured ? `Configured in ${rcFile}` : `Not found — run: source ~/${rcFile}`,
    critical: true,
  });
...
export function generateSummary(state: InstallState): InstallSummary {
  return {
    ...
    totalSteps: 8,
    shellRcFile: ((s) => s.includes("bash") ? ".bashrc" : s.includes("fish") ? ".config/fish/config.fish" : ".zshrc")(process.env.SHELL || "/bin/zsh"),
  };
}
